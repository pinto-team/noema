# -*- coding: utf-8 -*-
"""
NOEMA • control/policy.py — سیاست انتخاب عمل (V0 مینیمال، ماژولار)

هدف:
  - دریافت: State فعلی + مجموعه‌ی نامزدها (Actions) + تابع پیش‌بینی مدلِ جهان
  - خروجی: بهترین عمل (یا نمونه‌گیری نرم)، همراه با جزئیات امتیازدهی

طراحی:
  1) برای هر نامزد، از world.predict(s, a) سیگنال‌های آینده را می‌گیریم:
       (s1, z1_hat, rhat, risk_hat, u_hat)
     - rhat: تقریبِ «پیشرفت/پاداش مورد انتظار» (نقش r_int)
  2) با value.combine_rewards و shape_bonus امتیاز را می‌سازیم:
       r_total = w_int*rhat + w_ext*r_ext − λ*risk − μ*energy
       سپس شکل‌دهی با اعتمادبه‌نفس/عدم‌قطعیت
  3) انتخاب: argmax (یا softmax با دما)
...
یادداشت:
  - این ماژول سپر ایمنی را فراخوانی نمی‌کند (پیش از آن انجام شود).
  - اگر planner/candidates ماژول جدا دارید، می‌توانید از این‌جا فقط امتیازدهی را صدا بزنید.
"""
...
# ------------------------- پیکربندی انرژی پیش‌فرض -------------------------

_DEFAULT_ENERGY: Dict[str, float] = {
    # برآورد خیلی ساده، برحسب «هزینه‌ی نسبی»
    "reply_greeting": 0.02,
    "invoke_calc":   0.05,
    "ask_clarify":   0.03,
}
...
def score_candidate(
    ...
    *,
    ...
    """
    خروجی: (score, details)
      details شامل s1_conf, rhat, risk, u_hat, energy, base, shaped
    """
    s1, z1_hat, rhat, risk, u_hat = predict_fn(state, action)
    conf = float(getattr(s1, "conf", 1.0 - float(u_hat)))
    energy = _energy_of(action, energy_costs)
    base = combine_rewards(r_int=float(rhat), r_ext=float(r_ext), risk=float(risk), energy=float(energy), spec=spec)
...
def decide(
    ...
    *,
    ...
    """
    انتخاب بهترین عمل از میان نامزدها.
    - اگر candidates خالی باشد، ask_clarify برمی‌گرداند.
    - ranked_details: لیستی مرتب‌شده از بهترین به بدترین (برای دیباگ/لاگ).
    """
    ...
    scored: List[Tuple[float, Dict[str, Any], Action]] = []
    for a in candidates:
        sc, det = score_candidate(state, a, predict_fn, r_ext=r_ext, energy_costs=energy_costs, spec=spec)
        scored.append((sc, det, a))

    scored.sort(key=lambda t: t[0], reverse=True)
    scores = [s for s,_,_ in scored]

    if method == "softmax" and len(scored) > 1:
        probs = _softmax(scores, t=float(temperature))
        choice = random.choices(range(len(scored)), weights=probs, k=1)[0]
        best = scored[choice]
    else:
        best = scored[0]

    best_action = best[2]
    ranked_details = [det for _, det, _ in scored]
...
if __name__ == "__main__":
    ...
    # پیش‌بینی ساختگی: تغییر کم برای greet، متوسط برای calc، زیاد برای ناشناخته
